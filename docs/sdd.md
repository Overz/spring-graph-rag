...
|----------------------------------------------------------------|---------------------------------------------|----------------------------------------------------------------------|
| [`openspec/requirements/`](../openspec/requirements/)          | **O quê** o sistema deve fazer              | Fonte da verdade: RF-01–RF-40 + RNF-01–04, um arquivo por requisito  |
| [`openspec/plans/`](../openspec/plans/)                        | **Em que ordem e em que pedaços** construir | Backlog: épicos (PLAN-00–PLAN-10) ↔ RFs ↔ features BDD              |
| `sdd.md` + [`sdd/`](sdd/) (este)                                | **Como** cada pedaço é desenhado            | Design detalhado: C4, contratos, modelos de dados, decisões técnicas |
| [`openspec/decisions/`](../openspec/decisions/)                 | **Por que** decisões pontuais foram tomadas | ADRs com contexto e alternativas                                     |
| `src/test/resources/features/`                                  | **Quando** um requisito está pronto         | Critérios de aceite executáveis (Gherkin, tags `@RFxx`)              |

**Regra de coerência:** requisitos vencem o SDD; o SDD vence a memória — se a implementação divergir do desenho, ou o desenho estava errado (atualize-o) ou o código está (corrija-o). Decisões *por quê* significativas continuam indo para `openspec/decisions/`, referenciadas daqui.

## 2. Como este SDD foi produzido

O design foi definido em uma **sessão de descoberta arquitetural** (julho/2026) sobre as fontes oficiais — os requisitos, o plano de épicos, ADRs 001–003, a suíte BDD e o repositório — questionando premissas e registrando cada decisão no *Architecture Decision Log* (seção 4). As decisões marcadas "ADR pendente" ganham ADR formal quando o épico correspondente chegar.
...
|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|
| ADL-001 | Storage de artefatos atrás da porta `DocumentStorage`; JuiceFS + MinIO + Redis; estágios RAW/TRANSFORMED                                                                                                                                                    | [ADR-001](../openspec/decisions/ADR-001.md)             |
| ADL-002 | Docling Serve (CPU) para PDF/imagem/OCR; Tika para demais formatos e detecção de MIME                                                                                                                                                                       | [ADR-002](../openspec/decisions/ADR-002.md)             |
| ADL-003 | Embeddings via Ollama; `nomic-embed-text` inicial, validado contra golden set pt-BR/EN antes do volume                                                                                                                                                      | [ADR-003](../openspec/decisions/ADR-003.md) + [5.4]     |
| ADL-004 | NATS como broker externo, precedido pelos eventos internos do Modulith                                                                                                                                                                                      | Premissa 3; ADR pendente ([3.4])                        |
| ADL-005 | Defaults de retrieval: RRF `k=60`, travessia 1–2 hops, merge de entity resolution > 0.85 — pontos de partida ajustáveis por medição                                                                                                                         | [RF-25](../openspec/requirements/RF-25.md)/[RF-32](../openspec/requirements/RF-32.md) |
...
| ADL-010 | SDD físico: `sdd.md` índice + `docs/sdd/` por domínio, diagramas Mermaid, ADRs em `openspec/decisions/`                                                                                                                                                     | Descoberta                                              |
...
- **Status de infra:** serviços marcados *(pendente)* ainda não existem no `compose.yaml` — os [planos por épico](../openspec/plans/) dizem em qual entram.
