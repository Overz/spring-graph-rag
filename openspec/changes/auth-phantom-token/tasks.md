...
- [ ] 1.1 Confirmar D1 (módulo Spring Modulith novo `auth` vs. pacote em `api`) com o usuário
- [ ] 1.2 Confirmar path/nome definitivo do(s) endpoint(s) (`/api/v1/auth/login`, `/api/v1/auth/logout`?)
- [ ] 1.3 Decidir se abre ADR novo (`docs/adr/ADR-004-...`) para D1+D2 ou se fica só em design.md
...
## 3. Módulo/pacote `auth` (conforme decisão da tarefa 1.1)

- [ ] 3.1 Criar `package-info.java` com `@ApplicationModule` (se D1 confirmado) e `allowedDependencies` (`shared::errors`, `shared::logging`, `shared::security`, `shared::support`)
- [ ] 3.2 Interface `PhantomTokenIssuer` (contrato: emitir a partir de usuário/senha, revogar a partir do token opaco) — API pública do módulo
...
- [ ] 3.6 Erros de domínio (`errors/`, sufixo `Exception`): credenciais inválidas, token revogado/inexistente

## 4. Integração com o resource server (`api`)

- [ ] 4.1 `api` passa a depender de `auth` (`allowedDependencies += "auth"` em `api/package-info.java`, se D1 confirmado)
- [ ] 4.2 `AuthenticationManagerResolver`/filtro que distingue JWT (2 pontos) de token opaco (design.md D2)
- [ ] 4.3 `OpaqueTokenIntrospector` que consulta `PhantomTokenIssuer`/Redis e monta o mesmo formato de claims que `CallerContextJwtConverter` já produz para JWT
- [ ] 4.4 Confirmar que `CallerContext` final é idêntico nos dois caminhos (JWT direto e token opaco) — nenhuma mudança em controllers/serviços que já consomem `CallerContext`
- [ ] 4.5 Controller(s) HTTP: `POST /api/v1/auth/login`, `POST /api/v1/auth/logout` (se 1.4 confirmar escopo), dependendo só de `PhantomTokenIssuer`
...
- [ ] 6.3 Atualizar `CLAUDE.md` (Architecture/Module Dependency Rules, se módulo novo confirmado)
